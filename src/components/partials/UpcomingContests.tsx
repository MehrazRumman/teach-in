...
import TestCardSmall, { CardSkeleton } from '~/components/shared/TestCardSmall';
...
import ContestCardSmall from '../shared/ContestCardSmall';
function PopularTests() {
  ...
  const { data, status } = trpc.test.findContestsByFilters.useQuery({
    limit: 10,
    page: '1',
    sortBy: '',
  });
  ...
  return (
    <div className="my-6 h-fit w-full text-gray-800 dark:text-white">
      ...
      <div className="mx-auto my-4 px-3 md:px-6">
        <If condition={status === 'loading'}>
          <Then>
            <Swiper
              loop={data && data?.tests?.length > 7}
              onBeforeInit={(swiper) => {
...
//Student
export default memo(PopularTests);
